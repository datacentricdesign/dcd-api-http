...
exports.introspect = (req, res, next) => {
    ...
    if (req.get('Authorization') === undefined) {
        return next(new Error('Request not allowed '
    ...
    return model.auth.refresh().then(() => {
        ...
        if (token.split('.').length === 3
            ...
            return model.auth.checkJWTAuth(token, req.params.entityId)
                .then((token) => {
...
exports.wardenToken = ({resource, action, scope = []}) => (req, res, next) => {
    let acpResource = 'dcd';
    ...
    if (token.split('.').length === 3
        ...
        model.auth.checkJWT(acp, req.params.entityId)
            .then((user) => {
                if (user !== undefined) {
                    req.user = user;
                    next();
